...
namespace csc
{
	public partial class AE_CPPl
	{
		...
		public int CPPl_Size
		{
			get { return length_CPPl_cpid; }
		}
		...
		public string Info
		{
			get
			{
				string ret = $"({{RIFX:{value_RIFX},LIST:{value_LIST},";
				ret += $"CPPl:{adr_CPPl},cpid:{adr_cpid},CPPl_Size:{length_CPPl_cpid} }})";
				return ret;
			}
		}
		...
		// ********************************************************************************************
		private uint FindTagSize(string tag, int idx)
		{
			uint ret = 0;
			int i = Find(tag, idx);
			if (i < 0) return ret;
		...
		private void WriteUint(int idx, uint v)
		{
			if (idx < m_aep.Length - 4)
			{
